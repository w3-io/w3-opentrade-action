...
Read-only steps only need the bridge with an Avalanche RPC. Write steps additionally require a signer that OpenTrade has KYC'd and whitelisted on the target vault.
...
For write steps, additionally provide a KYC-whitelisted signer to the bridge:
